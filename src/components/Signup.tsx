...
const Signup = () => {
  ...
  useEffect(() => {
    ...
    script.onerror = () => {
      ...
      toast.error('Không thể tải xác thực captcha')
    }
  ...
  useEffect(() => {
    if (isTurnstileLoaded && !turnstileWidgetId && window.turnstile) {
      try {
        const widgetId = window.turnstile.render('#turnstile-container-signup', {
          ...
          'error-callback': () => {
            ...
            toast.error('Xác thực captcha thất bại')
          },
          'expired-callback': () => {
            setError('Captcha đã hết hạn, vui lòng thử lại')
            setTurnstileToken(null)
            toast.error('Captcha đã hết hạn, vui lòng thử lại')
          }
        })
        setTurnstileWidgetId(widgetId)
      } catch (err) {
        setError('Không thể khởi tạo captcha')
        toast.error('Không thể khởi tạo captcha')
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        const response = await comicApis.signup({
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className='space-y-6'>
        ...
        <div id='turnstile-container-signup' className='flex justify-center my-4'></div>
        ...
        >
          {isLoading ? (
            <div className='flex items-center space-x-2'>
              ...
              <span className='text-xs sm:text-sm'>Tạo tài khoản...</span>
            </div>
          ) : (
            'Đăng ký'
          )}
