...
const RegularNavItems = () => {
  const getNavLinkClass = ({ isActive }: { isActive: boolean }) =>
    `${NAV_LINK_BASE_STYLES} ${isActive ? 'text-primary' : ''}`

  const commonSearchParams = createSearchParams({ page: '1' }).toString()
    ...
    <>
      <li>
        <NavLink
          ...
          to={{ pathname: PATH.recent, search: commonSearchParams }}
      ...
      <li>
        <NavLink
          ...
          to={{ pathname: PATH.popular, search: commonSearchParams }}
          className={getNavLinkClass}
        >
          nổi bật
        </NavLink>
      ...
      <li>
        <NavLink
          ...
          to={{ pathname: PATH.completed, search: commonSearchParams }}
          className={getNavLinkClass}
        >
          đã hoàn thành
        </NavLink>
      ...
      <li>
        <NavLink
          ...
          to={{ pathname: PATH.boy, search: commonSearchParams }}
          className={getNavLinkClass}
        >
          con trai
        </NavLink>
      ...
      <li>
        <NavLink
          ...
          to={{ pathname: PATH.girl, search: commonSearchParams }}
          className={getNavLinkClass}
        >
          con gái
        </NavLink>
...
// Top nav items component
const TopNavItems = ({ pathname, queryConfig }: { pathname: string; queryConfig: any }) => {
  const getTopNavLinkClass = ({ isActive }: { isActive: boolean }) =>
    `${TOP_NAV_LINK_STYLES} ${isActive ? 'text-primary' : ''}`
...
const Navbar = () => {
  ...
  const containerClasses = classNames(
    'text-black/60 dark:text-white transition-transform duration-300 ease-in-out',
    {
      ...
      'transform translate-y-0': isVisible || isHome
    }
  )
    ...
    <div className={containerClasses}>
      <nav className={navClasses}>
        <ul className='flex items-center gap-3 overflow-x-auto overflow-y-hidden hide-horizontal-scrollbar'>
          ...
          {isTop && <TopNavItems pathname={pathname} queryConfig={queryConfig} />}
        </ul>
      </nav>
    </div>
  )
