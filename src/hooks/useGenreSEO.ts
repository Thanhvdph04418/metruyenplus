...
export const useGenreSEO = ({ currentGenre, type, totalPages }: UseGenreSEOParams) => {
  ...
  const canonicalUrl = useMemo(
    () => generateCanonicalUrl('/the-loai', ['type'], { type }),
    [type]
  )
  ...
  const prevUrl = useMemo(
    () =>
      showPrevLink
        ? generatePaginationUrl('/the-loai', currentPage - 1, ['type'], { type })
        : null,
  ...
  const metaTags = useMemo(
    () => ({
      ...
      description: `Truyện Thể loại ${currentGenre?.name || ''} - ${currentGenre?.description || ''}`,
      canonicalUrl,
      prevUrl,
      nextUrl
    }),
